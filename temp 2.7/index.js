...
function init() {
    ...
    light1.shadow.bias = -0.001; // 0.0001 creates cool lines
    light1.shadow.mapSize.width = Math.pow(2, 8);
    light1.shadow.mapSize.height = Math.pow(2, 8);
    ...
    var ambientLight = new THREE.AmbientLight('#aaa'); 
    ...
    var hemiLight = new THREE.HemisphereLight('#ccc', '#ccc', .35);
    ...
    loader.load( thePalm, function ( gltf ) {
        ...
        palmMesh.position.set(0, palmHeight, 6);
    ...
    loader.load( thePalm, function ( gltf ) {    
        ...
        palm2.position.set(0, palmHeight, 6);
    ...
    loader.load( thePalm, function ( gltf ) {    
        ...
        palm3.position.set(0, palmHeight, 6);
    ...
    saoPass.params.saoIntensity = .0007;
...
function animate() {
    ...
    // LIGHT1
    let offsetSin = 400;
    var lightSpeed = 0.005;
    light1.position.x = Math.cos( (offsetSin + timeElapsed) * lightSpeed) * 7;
    // light1.position.y = 16 + light1YOrbit * Math.sin(timeElapsed*lightSpeed);
    light1.position.z = 6 + Math.sin( (offsetSin + timeElapsed) * lightSpeed) * 7;
    ...
    // VERTEX COLORS
    // vColorSin += Math.sin(timeElapsed * .5 ) * 50;
    // wallNZMesh.material.needsUpdate = true;
